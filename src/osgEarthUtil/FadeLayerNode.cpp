...
char vert_source[] =
                    ...
                    "    vec3 normal = fnormal(); \n"
                    " \n"
                    "    if (osgEarth_lightsEnabled[0]) directionalLight(0, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[1]) directionalLight(1, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[2]) directionalLight(2, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[3]) directionalLight(3, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[4]) directionalLight(4, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[5]) directionalLight(5, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[6]) directionalLight(6, normal, ambient, diffuse, specular); \n"
                    "    if (osgEarth_lightsEnabled[7]) directionalLight(7, normal, ambient, diffuse, specular); \n"
...
char frag_source[] =
                    ...
                    "  int numLayersOn = 0;\n"
                    "  float maxAlpha = 0.0;\n"
                    "  if (osgearth_imagelayer_count>0 && osgearth_imagelayer_enabled[0])\n"
                    "  {\n"
                    "    vec4 texel = colors[0];\n"
                    "    float alpha = texel.a * osgearth_imagelayer_opacity[0];\n"
                    "    color = mix(color, texel.rgb, alpha);\n"
                    "    numLayersOn++;\n"
                    "    if (maxAlpha < alpha) maxAlpha = alpha;\n"
                    "  }\n"
                    "  if (osgearth_imagelayer_count>1 && osgearth_imagelayer_enabled[1])\n"
                    "  {\n"
                    "    vec4 texel = colors[1];\n"
                    "    float alpha = texel.a * osgearth_imagelayer_opacity[1];\n"
                    "    color = mix(color, texel.rgb, alpha);\n"
                    "    numLayersOn++;\n"
                    "    if (maxAlpha < alpha) maxAlpha = alpha;\n"
                    "  }\n"
                    "  if (osgearth_imagelayer_count>2 && osgearth_imagelayer_enabled[2])\n"
                    "  {\n"
                    "    vec4 texel = colors[2];\n"
                    "    float alpha = texel.a * osgearth_imagelayer_opacity[2];\n"
                    "    color = mix(color, texel.rgb, alpha);\n"
                    "    numLayersOn++;\n"
                    "    if (maxAlpha < alpha) maxAlpha = alpha;\n"
                    "  }\n"
                    "  if (osgearth_imagelayer_count>3 && osgearth_imagelayer_enabled[3])\n"
                    "  {\n"
                    "    vec4 texel = colors[3];\n"
                    "    float alpha = texel.a * osgearth_imagelayer_opacity[3];\n"
                    "    color = mix(color, texel.rgb, alpha);\n"
                    "    numLayersOn++;\n"
                    "    if (maxAlpha < alpha) maxAlpha = alpha;\n"
                    "  }\n"
