...
SLDReader::readStyleFromCSSParams( const Config& conf, Style& sc )
{
    ...
    for(Properties::const_iterator p = conf.attrs().begin(); p != conf.attrs().end(); p++ )
    {
		...
		else if (p->first == CSS_TEXT_ENCODING)
		{
			if (!text) text = sc.getOrCreateSymbol<TextSymbol>();
			if (p->second == "utf-8") text->encoding() = TextSymbol::ENCODING_UTF8;
			else if (p->second == "utf-16") text->encoding() = TextSymbol::ENCODING_UTF16;
			else if (p->second == "utf-32") text->encoding() = TextSymbol::ENCODING_UTF32;
			else if (p->second == "ascii") text->encoding() = TextSymbol::ENCODING_ASCII;
